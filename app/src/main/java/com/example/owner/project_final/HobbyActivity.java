...
import com.example.owner.project_final.adapter.HobbyAdapter;
...
public class HobbyActivity extends AppCompatActivity {
    ...
    // For Activity finish -------------------------------------------------------------------------
    public static Activity hobbyActivity;
    HobbyActivity hobbyact = (HobbyActivity) HobbyActivity.hobbyActivity;
    //----------------------------------------------------------------------------------------------
    ...
    private void onLoad() {
        ...
        Query req = reference.child(PublicVariable.FIREBASE_CHILD_HOBBIES);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    for (String userKey : UserKeyList) {
                        DatabaseReference qurey = FirebaseDatabase.getInstance().getReference().child(PublicVariable.FIREBASE_CHILD_HOBBIES).child(userKey);
                        qurey.addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                if (dataSnapshot.exists()) {
                                    for (DataSnapshot child : dataSnapshot.getChildren()) {
                                        HobbyWrite item = child.getValue(HobbyWrite.class);
                                        mHobbyAdapter.insertData(item);
                                    }
                                }
                            }
...
*/
        ...
        mHobbyAdapter = new HobbyAdapter(getApplicationContext()) {
            @Override
            public void selectItem(HobbyWrite item) {
                L.e(":::::Click item : " + item.toString());
                Intent intent = new Intent(HobbyActivity.this, HobbyDetailActivity.class);
                intent.putExtra("HobbyWrite", item);
                startActivity(intent);
            }
        };
        mRecyclerView.setLayoutManager(new LinearLayoutManager(getApplicationContext()));
        mRecyclerView.setAdapter(mHobbyAdapter);
        onLoad();
    }
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_write_post:
                ...
                if (!gpsEnabled()) {
                    ...
                    new AlertDialog.Builder(HobbyActivity.this, R.style.Theme_AppCompat_Light_Dialog)
                    ...
                    Dexter.withActivity(HobbyActivity.this).withPermissions(Manifest.permission.ACCESS_COARSE_LOCATION, Manifest.permission.ACCESS_FINE_LOCATION).withListener(new MultiplePermissionsListener() {
                        @Override
                        public void onPermissionsChecked(MultiplePermissionsReport report) {
                            if (report.areAllPermissionsGranted()) {
                                intent = new Intent().setClass(HobbyActivity.this, HobbyWriteActivity.class);
                                startActivity(intent);
